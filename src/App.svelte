<script>
  ...
  import { getMockMoneyStories } from './lib/moneyStories.mock.js';
  ...
  async function loadStories() { storiesSection = { ...storiesSection, status: storiesSection.data ? 'refreshing' : 'loading', error: '' }; try { storiesSection = { status: 'ready', data: useMockMoneyStories ? getMockMoneyStories(selectedMonth) : await getMoneyStories(selectedMonth), error: '' }; } catch (cause) { if (!(cause instanceof ApiError && cause.status === 401)) storiesSection = { ...storiesSection, status: 'error', error: cause?.message || 'Could not load Money Stories.' }; } }
