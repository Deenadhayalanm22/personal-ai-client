...
{#if !openedStory}<section class="top-greeting"><section class="app-heading"><div><p class="micro-label">{monthName(selectedMonth).toUpperCase()}</p><h1>{greeting()}</h1><p>Your recorded spending, one day at a time.</p></div><button class="profile-button" on:click={signOut} disabled={loggingOut} aria-label="Sign out">{loggingOut?'…':'D'}</button></section>{#if signOutError}<div class="notice error">{signOutError}</div>{/if}</section>{/if}
{#if !openedStory}<div class="top-story-rail"><SectionState section={storiesSection} title="Money Stories" retry={refreshStories}><section class="story-rail-section"><div class="section-title"><div><p class="micro-label">MONEY STORIES</p><h2>{monthLabel(selectedMonth)} patterns</h2></div><span>{stories.length} stories</span></div><div class="story-rail" aria-label="Money Stories">{#each stories as story}<button class:viewed={storyWasViewed(story)} class="story-orb" on:click={()=>openStory(story)}><span class="story-ring" style={storyRingStyle(story)}><span class="story-orb-inner">{storyInitial(story)}</span></span><strong>{storyLabel(story)}</strong><small>{story.cards?.length || 1} cards</small></button>{:else}<div class="story-rail-empty">No stories were created for this month.</div>{/each}</div></section></SectionState></div>{/if}
{#if !openedStory}<div class="workspace-divider" aria-hidden="true"><span>YOUR SPENDING LOG</span></div>{/if}
{#if !openedStory}<nav class="primary-view-nav" aria-label="Main sections"><button class:active={activeView==='calendar'} on:click={()=>activeView='calendar'}><span>▦</span> Calendar</button><button class:active={activeView==='normalization'} on:click={()=>activeView='normalization'}><span>≋</span> Normalization</button></nav>{/if}
