<script>
  ...
  async function saveEdit(){const amount=Number(editAmount),merchantId=Number(editMerchantId),accountId=Number(editAccountId);if(!editing||saving||optionsStatus!=='ready'||!Number.isFinite(amount)||amount<=0||!editDate||!editCategory||!editSubcategory||!Number.isFinite(merchantId)||!Number.isFinite(accountId))return;saving=true;actionError='';try{const updated=await updateExpense(editing.id,{amount,transactionDate:editDate,category:editCategory,subcategory:editSubcategory,merchantId,accountId});dayItems=dayItems.filter(x=>x.id!==updated.id||editDate===isoDate(selectedDay));if(editDate===isoDate(selectedDay))dayItems=dayItems.map(x=>x.id===updated.id?updated:x);editing=null;await Promise.all([loadDay(),refreshRecent(),refreshCalendar(),refreshStories()]);}catch(cause){actionError=cause?.message||'Could not update the transaction.';}finally{saving=false}}
